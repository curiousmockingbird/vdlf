	...
	<section class=" min-h-screen bg-gray-100 pb-10" id="filter-section">
		<div class="container">
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-10 gap-x-7 my-10" v-if="updates && updates.length>0">
				<a :href="item.link?item.link:'#'"	 v-for="(item, i) in updates" :key="i" class="mention-card">
					<div class="p-7">
					...
					<button target="_blank" class="text-left text-black py-8 px-7 rounded-b-md transition-all bg-themeBlue text-white mt-auto font-bold underline text-xl block">Continue Reading</button>
				</a>
			</div>
...
<style lang="scss">
.mention-card {
	@apply cursor-pointer flex flex-col h-full rounded-lg hover:shadow-2xl bg-white transition-shadow;
	&:hover {
